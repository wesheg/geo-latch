...
float haversine_distance(float lat1, float long1, float lat2, float long2) {
  ...
  float a =
      pow(sin(dlat / 2), 2) + cos(lat1R) * cos(lat2R) * pow(sin(dlon / 2), 2);
...
/**
 * Calculates the distance in miles between two coordinates on the same
 * longitude.
 *
 * @param lat1 Latitude of the first coordinate in decimal degrees.
 * @param lat2 Latitude of the second coordinate in decimal degrees.
...
/**
 * Determines the cardinal direction from the current coordinate to the
 ...
 * @param latitude A boolean indicating if the coordinates are latitude (true)
 * or longitude (false).
 * @return A string representing the cardinal direction ("North", "South",
 * "East", or "West").
 */
String cardinal_direction(float current_coordinate,
                          float destination_coordinate, bool latitude) {
  if (latitude) {
 ...
 */
void unlock_box() { myservo.write(unlocked); }
...
/**
 * Displays the welcome messages on the LCD at startup.
 */
void lcd_start() {
...
/**
 * Prints to the LCD without clearing the whole console to prevent blinking.
 *
 * @param row1_new The new string to print on the first row.
 * @param row2_new The new string to print on the second row.
 */
...
/**
 * Converts Adafruit GPS coordinates to decimal degrees.
 *
 * @param coordinate The GPS coordinate in DDMM.MMMMM or DDDMM.MMMMM format.
 * @param cardinal The cardinal direction ("N", "S", "E", "W").
...
void setup() {
  ...
  GPS.begin(9600);
  GPS.sendCommand(
      PMTK_SET_NMEA_OUTPUT_RMCGGA);  // RMC (recommended minimum) and GGA (fix
...
void loop() {
  ...
  if (GPS.newNMEAreceived()) {
    if (!GPS.parse(
            GPS.lastNMEA()))  // also sets the newNMEAreceived() flag to false
  ...
  if (GPS.fix) {
    ...
    float y =
        haversine_x_distance(current_long, destination_long, destination_lat);
    float x =
        haversine_y_distance(current_lat, destination_lat, destination_long);

    String x_direction = cardinal_direction(current_lat, destination_lat, true);
    String y_direction =
        cardinal_direction(current_long, destination_long, false);
